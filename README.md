...
**Important:** Before running the application, you must set up environment variables. Copy the example files and fill in your actual values:
```bash
# Root directory
cp .env.example .env
...
See [.env.example](.env.example), [backend/.env.example](backend/.env.example), and [frontend/.env.example](frontend/.env.example) for required variables.
...
<image src="./github_assets/login_modal.png" height="300px"> <image src="./github_assets/registration_modal.png" height="300px">
...
<video src="https://github.com/user-attachments/assets/361b126f-d1e9-4e0e-96d1-ef983c97dd32" controls></video>
...
3. Store actual credentials in `.env` files (git-ignored)
4. For production, use environment variables or secret management services

**If you accidentally committed secrets:**
1. Immediately rotate all exposed keys (Firebase, Stripe, database passwords)
...
### Authentication Flow
Firebase handles user authentication. Backend verifies Firebase ID tokens on protected endpoints.
